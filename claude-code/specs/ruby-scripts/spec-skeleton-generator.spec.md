...
- **NEW in v2.0:** Generates `let` blocks for characteristic states (shadowing pattern)
...
5. **Generate `let` blocks for characteristic states** (binary/enum/sequential)
6. Insert placeholders for architect and implementer agents
7. Write complete RSpec skeleton to file
...
def generate_context_code(context_tree, indent_level = 2)
  ...
  context_tree.each do |context|
    ...
    # Generate let blocks for states (NEW: shadowing pattern)
    # This enables factory/implementer to reference characteristic.name variable
    if context[:characteristic]
      char = context[:characteristic]
      state = context[:state]
    ...
    # Add {SETUP_CODE} placeholder for factory/implementer
    # (will be filled with let(:model) or let(:params) blocks)
...
def format_state_value(state)
  # Convert state name to appropriate Ruby value for let blocks
  # Binary: first state → true, second → false
  # Enum/Sequential: state name → :symbol

  # Binary state detection: if state ends with negation prefix
  if state =~ /^not_/
    'false'
  elsif state =~ /^(enabled|authenticated|active|valid|confirmed|approved)$/
    'true'
  else
    # Enum/Sequential: convert to symbol
    ":#{state}"
  end
end
```

**Shadowing Pattern Explanation:**

The skeleton generator creates `let` blocks for characteristic states to enable **shadowing pattern** in RSpec:

1. **Skeleton** generates: `let(:characteristic_name) { state_value }`
2. **Factory/Implementer** references this variable in model creation:
   ```ruby
   let(:user) { build_stubbed(:user, authenticated: authenticated) }
   ```
3. When state changes in child context, the `let` is **shadowed** (overridden), and factory automatically gets new value

**Example:**
```ruby
# characteristic: { name: 'authenticated', type: 'binary', states: [authenticated, not_authenticated] }

context 'when authenticated' do
  let(:authenticated) { true }           # ← Generated by skeleton
  let(:user) { build_stubbed(:user, authenticated: authenticated) }  # ← Generated by factory
...
context 'when NOT authenticated' do
  let(:authenticated) { false }          # ← Generated by skeleton (shadows parent)
...
RSpec.describe UserService do
  describe '#activate' do
    ...
    context 'when user active' do  # Rule 20: 'when' at level 1 (opens branch)
      let(:user_active) { true }  # ← NEW: Shadowing pattern for binary characteristic
      {SETUP_CODE}
    ...
    context 'when user inactive' do  # Rule 20: 'when' at level 1
      let(:user_active) { false }  # ← NEW: Shadowing pattern (false for second state)
      {SETUP_CODE}
...
RSpec.describe PaymentService do
  describe '#process' do
    ...
    context 'when payment method is card' do  # Rule 20: 'when' at level 1, enum uses 'and' within level
      let(:payment_method) { :card }  # ← NEW: Enum uses symbols
      {SETUP_CODE}
    ...
    context 'when payment method is paypal' do  # Rule 20: enum continues with 'and' (shown if nested)
      let(:payment_method) { :paypal }  # ← NEW: Shadowing pattern with symbol
      {SETUP_CODE}
    ...
    context 'when payment method is bank transfer' do  # Rule 19: snake_case → spaces ("bank_transfer" → "bank transfer")
      let(:payment_method) { :bank_transfer }  # ← NEW: State name converted to symbol
      {SETUP_CODE}
...
RSpec.describe PaymentService do
  describe '#process_payment' do
    ...
    context 'when user authenticated' do  # Rule 20: 'when' at level 1 (opens branch)
      ...
      let(:user_authenticated) { true }  # ← NEW: Shadowing pattern for binary
      {SETUP_CODE}
      ...
      context 'and payment method is card' do  # Rule 20: 'and' for enum at level 2 (continues happy path)
        ...
        let(:payment_method) { :card }  # ← NEW: Nested shadowing (inherits user_authenticated = true)
        {SETUP_CODE}
      ...
      context 'and payment method is paypal' do  # Rule 20: 'and' continues enum states
        # Logic: app/services/payment_service.rb:52-56
        let(:payment_method) { :paypal }  # ← NEW: Different payment_method, same user_authenticated
        {SETUP_CODE}
    ...
    context 'when user NOT authenticated' do  # Rule 19: NOT capitalized for emphasis (Rule 7: negative state last)
      # Logic: app/services/payment_service.rb:45
      let(:user_authenticated) { false }  # ← NEW: Shadowing changes value to false
      {SETUP_CODE}
