...
2. **Генерация shared_examples для повторяющихся behavior**
   ...
   - Имя shared*examples должно быть стабильным и уникальным *в рамках метода\_, например:
...
- после test-architect мы не проверяем валидность спецификации, так как скрипт достаточно детерминирован
- test-implementer не оставляет metadata файлов, пока что не будет обрабатывать этот случай.

2. Встроить self-check loop в агентов:

- `isolation-decider`, `test-architect` (если требуется), `code-analyzer`, `discovery-agent`
  - агент делает работу,
  - запускает валидатор,
...
3. Вписать это в rule docs/PLUGINS-GUIDE.md

4. В этот же этап добавить “сигнал комбинаторного взрыва”:

- Валидатор (или code-analyzer) считает количество характеристик/ветвей.
- Если > 4–5 характеристик или оценка листьев > порога — `AskUserQuestion` (продолжить/приостановить), рефакторинг или правки в коде мы не предлагаем, это не задача этого плагина, суть в том чтобы предложить пользователю выбор и решить проблему самостоятельно до того как генерация дерева начнётся.
