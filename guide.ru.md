...
- **Happy path** — последовательность контекстов и пример, демонстрирующие нормальный ход событий (основную бизнес-цепочку) без исключений.
- **Corner case** — сценарий, выходящий за рамки основного потока (ошибочные данные, крайние значения, исключительные ситуации), проверяющий устойчивость поведения.
- **Положительный тест** — пример, подтверждающий поведение при выполнении всех нужных состояний характеристик (обычно это happy path).
- **Отрицательный тест** — пример, показывающий, что поведение не выполняется, когда хотя бы одно состояние характеристики нарушено (часто это corner case).
