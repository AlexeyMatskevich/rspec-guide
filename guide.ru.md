...
- **Do One Thing (Делай одно дело)** — принцип из "Clean Code" (Robert Martin): функция/метод/класс должен иметь одну ответственность и делать только одно дело на одном уровне абстракции.
...
- **Single Responsibility Principle (Принцип единственной ответственности)** — класс должен иметь только одну причину для изменения. Часть SOLID принципов.
...
- **Инкапсуляция** — сокрытие внутренних деталей реализации за публичным интерфейсом. Объект управляет своим состоянием и не выставляет внутренности наружу.
...
- **Dependency Injection (Внедрение зависимостей)** — зависимости передаются объекту извне (через конструктор, setter, параметр метода), а не создаются внутри объекта.
...
- **Tight Coupling (Тесное связывание)** — ситуация, когда классы/модули слишком сильно зависят друг от друга, знают о внутренних деталях друг друга.
...
- **Утечка абстракций (Leaky Abstraction)** — когда детали нижнего уровня "протекают" через интерфейс верхнего уровня. Пользователь абстракции вынужден знать детали реализации.

  **Сигнал в тестах:** Приходится знать детали схемы БД для работы с доменной моделью, бизнес-логика смешана с SQL, публичный API не выражает бизнес-операции.
  ...
  **Решение:** Чётко разделите слои (domain → persistence), доменный слой не должен знать о деталях хранения.
...
- **Unit-тесты** (модели, сервисы) — проверяют **все комбинации** характеристик детально
...
```ruby
# плохо: дублируем детальное тестирование всех комбинаций (это задача unit-тестов)
...
**Объединяйте детали внутри одного бизнес-домена.** На интеграционном уровне множество условий, относящихся к одной области ответственности (verified card + sufficient balance = payment prerequisites), объединяются в единое состояние. Мы проверяем что API работает в нормальном случае, а не комбинаторику всех возможных нарушений внутри домена.

**Маркер объединения:** если для домена существует отдельный unit-тест (например, `PaymentService`), значит в request spec детали этого домена можно объединить в единый контекст.
...
Каждый уровень вложенности = переход к следующему домену/слою ответственности.
...
- Все corner cases внутри домена обрабатываются **аналогичным образом** (например, возвращают ошибку валидации в HTTP-ответе одинаково)
- Меняется только **содержимое** (текст ошибки), а не поведение системы
- Проверка всех комбинаций = **дублирование unit-тестов** на интеграционном уровне
...
Детальное тестирование всех граничных условий и их комбинаций остаётся на более низких уровнях пирамиды в unit-тестах.
...
**См. также:** [Правило 10: Указывайте subject](#10-указывайте-subject-чтобы-явно-обозначить-предмет-тестирования) — явный `subject` в сочетании с чёткой иерархией контекстов делает тест максимально читаемым
...
- Нужно знать детали схемы БД для тестовых данных → доменный слой не изолирован от персистентности
...
1. **Идентифицируйте характеристики:**
   ...
   - Определите, какие из них — реальные [характеристики](#характеристики-и-состояния), а какие — технические детали
