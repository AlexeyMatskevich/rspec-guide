...
- Когда нужно проверить несколько следствий одного правила, разделяйте их на разные `it`. `:aggregate_failures` используйте лишь когда действительно говорите об одном поведении (см. пункт 18).
...
> Антипример: если вынести уточняющую характеристику на верхний уровень, контекст перестанет быть самодостаточным и потеряет связь с базовым контекстом.
>
> ```ruby
> # плохо
> describe '#purchase' do
...
Нарочно плохой пример: на первый взгляд контекстная иерархия соблюдена (сначала сегмент, потом уровень лояльности), но повторяющийся `let(:currency)` оставляет одну комбинацию без теста.

```ruby
# плохо: повторяющийся let(:currency) маскирует незакрытое состояние характеристики `currency`
...
Хороший пример: валюта вынесена на свой уровень, контексты читаются как цельные предложения, happy path идёт первым, а corner case проверяется отдельно.

```ruby
# хорошо: иерархия повторяет зависимости характеристик, а happy path и corner case покрыты явно
describe Billing::DiscountEvaluator do
  subject(:discount) { described_class.call(order) }
...
### 12. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны составлять валидное предложение на английском языке.
...
### 13. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны быть написаны так, чтобы их понимал любой человек
...
### 14. Грамматика формулировок в describe/context/it
...
### 15. Язык контекстов: when / with / and / without / but / NOT
...
### 16. Изучите подробно правила из rubocop по части наименования https://rspec.rubystyle.guide/#naming

## Инструменты и поддержка тестов

### 17. Не используйте [any_instance](https://rspec.info/features/3-13/rspec-mocks/old-syntax/any-instance/), allow_any_instance_of, expect_any_instance_of
...
### 18. Используйте `:aggregate_failures` только когда описываете одно правило
...
### 19. Предпочитайте verifying doubles (`instance_double`, `class_double`, `object_double`)
...
### 20. Используйте shared examples для проверки разделяемого поведения

`shared_examples` служат для проверки общего поведения разных объектов. Они не про DRY ради снижения строк кода — тесты мы не «программируем» (см. пункт 11), они описывают правила. Если поведение повторяется, выносите именно его описание и ожидаемые наблюдения.
...
### 21. Предпочитайте Request (API-контракт) specs вместо controller specs
...
### 22. Стабилизируйте время через `ActiveSupport::Testing::TimeHelpers`
...
### 23. Делайте вывод падения теста читаемым
...
- Если matcher не даёт достаточной ясности, напишите helper, который вернёт компактное описание расхождения (но не превращайтесь в mini-программу — см. пункт 11).
