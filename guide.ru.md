...
- `:aggregate_failures` (правило 20) применяем, только если говорим об одном поведении и хотим увидеть все нарушения сразу — это не способ проверить половину пирамиды в одном `it`.
...
- Когда нужно проверить несколько следствий одного правила, разделяйте их на разные `it`. `:aggregate_failures` используйте лишь когда действительно говорите об одном поведении (см. пункт 20).
...
4. Вынесите инвариантные ожидания в `shared_examples` и подключите их через `it_behaves_like` (см. пункт 22.2).
...
### 11. Используйте `attributes_for` для генерации параметров, которые не являются важной деталью в тестировании поведения
...
### 12. В юнит-тестах (кроме моделей) используйте `build_stubbed`
...
### 14. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны составлять валидное предложение на английском языке.
...
### 15. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны быть написаны так, чтобы их понимал любой человек
...
### 16. Грамматика формулировок в describe/context/it
...
### 17. Язык контекстов: when / with / and / without / but / NOT
...
### 18. Изучите подробно правила из rubocop по части наименования https://rspec.rubystyle.guide/#naming

## Инструменты и поддержка тестов

### 19. Не используйте [any_instance](https://rspec.info/features/3-13/rspec-mocks/old-syntax/any-instance/), allow_any_instance_of, expect_any_instance_of
...
### 20. Используйте `:aggregate_failures` только когда описываете одно правило
...
### 22. Используйте shared examples для декларации контрактов

`shared_examples` служат для декларации контрактов — ожиданий, которые повторяются в разных местах. Они не про DRY ради снижения строк кода — тесты мы не «программируем» (см. пункт 13), они описывают правила. Если ожидание повторяется, выносите именно его описание и наблюдаемые последствия.

Существует два сценария использования shared examples:

#### 22.1. Для общего поведения разных объектов
...
#### 22.2. Для инвариантных ожиданий внутри одного теста
...
### 23. Предпочитайте Request (API-контракт) specs вместо controller specs
...
### 24. Стабилизируйте время через `ActiveSupport::Testing::TimeHelpers`
...
### 25. Делайте вывод падения теста читаемым
...
- Если matcher не даёт достаточной ясности, напишите helper, который вернёт компактное описание расхождения (но не превращайтесь в mini-программу — см. пункт 13).
