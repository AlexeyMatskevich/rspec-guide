...
- Проверяйте на том уровне, где поведение естественно наблюдать. Request (API-контракт) spec проверяет статус/ответ, а не содержимое базы — иначе вы тестируете реализацию контроллера, а не API-контракт (см. правило 3).
...
- `:aggregate_failures` (правило 22) применяем, только если говорим об одном поведении и хотим увидеть все нарушения сразу — это не способ проверить половину пирамиды в одном `it`.
...
- **Задание состояния (`let`/`before`)** — объявление, которое делает формулировку контекста истинной. Его размещают сразу под соответствующим `context`, чтобы связь между описанием и подготовкой контекста читалась без поиска по файлу (см. пункт 10).
...
- Когда нужно проверить несколько следствий одного правила, разделяйте их на разные `it`. `:aggregate_failures` используйте лишь когда действительно говорите об одном поведении (см. пункт 22).
...
Каждый раз, как вы закончили работу над тестами, убедитесь, что структура `describe/context` реально соответствует характеристикам и их состояниям из [глоссария](#глоссарий) (см. пункт 5). Финальный аудит включает два типа проверок: дубликаты `let`/`before` выдают пропущенные состояния характеристик, а дубликаты `it` с одинаковыми ожиданиями выдают инвариантные контракты интерфейса.
...
4. Вынесите инвариантные ожидания в `shared_examples` и подключите их через `it_behaves_like` (см. пункт 24.2).
...
### 10. Каждый тест должен быть разделен на 3 этапа в строгой последовательности
...
### 11. Используйте возможности FactoryBot для скрытия деталей исходных данных
...
### 12. Используйте `attributes_for` для генерации параметров, которые не являются важной деталью в тестировании поведения

При тестировании поведения часто важен сам факт выполнения действия (создание заказа, обновление профиля), а не конкретные значения большинства атрибутов. `attributes_for` позволяет сгенерировать валидный хэш параметров из фабрики, снижая когнитивную нагрузку при чтении теста и избегая дублирования между фабрикой и тестом.
...
- Когнитивная нагрузка: читатель вынужден анализировать каждый атрибут, хотя они не влияют на проверяемое поведение
...
- Снижена когнитивная нагрузка: не нужно анализировать несущественные детали
...
**Золотое правило:** Если для понимания теста читатель должен открыть фабрику — значит, критичные параметры лучше прописать явно в тесте. Используйте `attributes_for` для снижения когнитивной нагрузки и дублирования, но не в ущерб читаемости.
### 13. В юнит-тестах (кроме моделей) используйте `build_stubbed`
...
Конечно, это не значит отказ от `let`, фабрик или shared contexts — они как раз помогают декларативно описать характеристики. Но если абстракция скрывает важную деталь проверки или заставляет "исполнять код в голове", лучше написать явно.
...
### 16. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны составлять валидное предложение на английском языке.
...
### 17. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны быть написаны так, чтобы их понимал любой человек
...
### 18. Грамматика формулировок в describe/context/it
...
### 19. Язык контекстов: when / with / and / without / but / NOT
...
### 20. Изучите подробно правила из rubocop по части наименования https://rspec.rubystyle.guide/#naming

## Инструменты и поддержка тестов

### 21. Не используйте [any_instance](https://rspec.info/features/3-13/rspec-mocks/old-syntax/any-instance/), allow_any_instance_of, expect_any_instance_of
...
### 22. Используйте `:aggregate_failures` только когда описываете одно правило
...
### 23. Предпочитайте verifying doubles (`instance_double`, `class_double`, `object_double`)
...
### 24. Используйте shared examples для декларации контрактов

`shared_examples` служат для декларации контрактов — ожиданий, которые повторяются в разных местах. Они не про DRY ради снижения строк кода — тесты мы не «программируем» (см. пункт 14), они описывают правила. Если ожидание повторяется, выносите именно его описание и наблюдаемые последствия.
...
### 25. Предпочитайте Request (API-контракт) specs вместо controller specs
...
### 26. Стабилизируйте время через `ActiveSupport::Testing::TimeHelpers`
...
### 27. Делайте вывод падения теста читаемым
...
- Если matcher не даёт достаточной ясности, напишите helper, который вернёт компактное описание расхождения (но не превращайтесь в mini-программу — см. пункт 14).
