...
**Примечание:** Для тестирования API-контрактов (структура полей, типы, обязательность) см. [guide.api.ru.md](guide.api.ru.md) — RSpec не лучший инструмент для этой задачи.
...
*Типы характеристик* (определяют количество возможных состояний):
...
- **[Правило 1](#1-тестируйте-поведение-а-не-реализацию)** (поведение, не реализация) — устраняет необходимость понимать внутреннее устройство кода
- **[Правило 5](#5-стройте-иерархию-context-по-зависимостям-характеристик-happy-path--corner-cases)** (иерархия контекстов) — делает зависимости характеристик видимыми сразу
- **[Правило 10](#10-указывайте-subject-чтобы-явно-обозначить-предмет-тестирования)** (subject) — явно показывает, что тестируется, не нужно искать объект в ожиданиях
- **[Правило 11](#11-каждый-тест-должен-быть-разделен-на-3-этапа-в-строгой-последовательности)** (трёхэтапная структура) — создаёт предсказуемый паттерн чтения
- **[Правила 12-13](#12-используйте-возможности-factorybot-для-скрытия-деталей-исходных-данных)** (фабрики) — скрывают технические детали, оставляют только бизнес-характеристики
- **[Правило 16](#16-явность-важнее-dry)** (явность важнее DRY) — тесты остаются документацией, а не головоломкой из абстракций
...
- **[Правило 23](#23-используйте-aggregate_failures-только-когда-описываете-одно-правило)** (aggregate_failures) — показывает все нарушения сразу, экономя циклы отладки
- **[Правило 28](#28-делайте-вывод-падения-теста-читаемым)** (читаемый вывод падения) — использование правильных матчеров даёт понятный и релевантный вывод при падении
...
describe "#some_action" do
  ...
  it "true" do          # "true"? Что true? Почему true? При каких условиях true? Читатель в недоумении...
...
describe "#some_action" do
  ...
  it "allows unlocking the user" do         # Ага! Теперь ясно: true означает "разблокировка разрешена"
    expect(some_action).to be(true)
  end
end
```
...
**См. также:** [Тестирование API-контрактов](guide.api.ru.md) — для HTTP API используйте специализированные инструменты вместо множественных `expect`.
...
2. **Активный залог в `it`, третье лицо.** Субъектом предложения выступает объект системы. Используйте глаголы действия для поведения: `order generates invoice`, `service authenticates user`. Для описания результирующего состояния используйте глаголы состояния: `order has parent`, `result is valid`, `record belongs to user`. Так читающий понимает, что именно проверяется, и предложение остается коротким.
...
- **`and …`** — добавляет ещё одно положительное состояние в том же направлении. Это **уточняющий контекст**, который продолжает детализацию предыдущего уровня. Можно использовать несколько подряд, пока ветка остаётся частью happy path: `context 'and balance covers the price'`.
...
- **`but …`** — подчёркивает противопоставление happy path. Это **уточняющий контекст**, который показывает отклонение от базового состояния. Часто применяется, когда happy path основан на состоянии по умолчанию (отдельный `with`-контекст не нужен). Контекст `but …` обязан содержать тест, показывающий, как меняется поведение, когда базовый контекст перестаёт выполняться: `context 'but balance does NOT cover the price'`.
...
**Примечание:** Для HTTP API существуют более подходящие инструменты, чем множественные `expect`. См. [Тестирование API-контрактов](guide.api.ru.md).
...
**Навигация по правилу:**
- [25.1. Для общего поведения разных объектов](#251-для-общего-поведения-разных-объектов)
- [25.2. Для инвариантных ожиданий внутри одного теста](#252-для-инвариантных-ожиданий-внутри-одного-теста)
...
**Как это снижает [когнитивную нагрузку](#почему-мы-пишем-тесты-именно-так-когнитивная-нагрузка):** Правильно подобранные матчеры и структура теста создают вывод, который сразу показывает суть проблемы — не нужно тратить время на расшифровку того, что пошло не так.
...
- Если matcher не даёт достаточной ясности, напишите helper, который вернёт компактное описание расхождения (но не превращайтесь в mini-программу — см. [правило 15](#15-не-программируйте-в-тестах)).

**См. также:** [Правило 23](#23-используйте-aggregate_failures-только-когда-описываете-одно-правило) — aggregate_failures помогает увидеть все проблемы сразу, улучшая читаемость отладки.
