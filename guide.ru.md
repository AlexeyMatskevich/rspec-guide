...
- Проверяйте на том уровне, где поведение естественно наблюдать. Request (API-контракт) spec проверяет статус/ответ, а не содержимое базы — иначе вы тестируете реализацию контроллера, а не API-контракт (см. правило 1).
...
- **rspec-openapi** — автоматическая генерация OpenAPI из Request (API-контракт) specs
...
Это просто хорошая практика, которая делает когнитивно проще изучать тест. Когда `subject` явно объявлен, читатель сразу видит, что именно проверяется, и не тратит время на поиск объекта тестирования в ожиданиях.
...
Явный `subject` снижает когнитивную нагрузку и делает структуру теста более предсказуемой.
...
FactoryBot помогает описывать [характеристики](#глоссарий) доменных объектов через трейты и параметризованные хэши, поэтому используем его, чтобы тесты говорили о поведении, а не о технических атрибутах.
...
#### Основной use case: Request (API-контракт) specs

Request (API-контракт) specs — наиболее частое место применения `attributes_for`, так как API-эндпоинты принимают хэши параметров.
...
### 26. Предпочитайте Request (API-контракт) specs вместо controller specs
...
- Если приходится поддерживать legacy controller specs, помечайте их как наследие (например, `describe SomeController, :legacy`) и планируйте миграцию. При доработках расширяйте по пирамиде: поведение — в Request (API-контракт) spec, мелкую логику выносите в сервис/модель и покрывайте юнитами.
- Не дублируйте проверки: если действие уже описано на уровне Request (API-контракт) spec, controller spec лишь повторит реализацию и будет ломаться при рефакторинге маршрутов или фильтров.
...
- Используйте RSpec Request (API-контракт) specs для поведения: HTTP-статусы, ключевые поля и побочные эффекты.
