...
- `:aggregate_failures` (правило 17) применяем, только если говорим об одном поведении и хотим увидеть все нарушения сразу — это не способ проверить половину пирамиды в одном `it`.
...
# хорошо: сравниваем состав коллекции
expect(some_action).to match_array [2, 3, 1] # pass

# хорошо: строго сравниваем значения без ссылок на форму объектов
...
- Когда нужно проверить несколько следствий одного правила, разделяйте их на разные `it`. `:aggregate_failures` используйте лишь когда действительно говорите об одном поведении (см. пункт 17).
...
### 5. Располагайте happy path перед corner cases
...
### 6. Пишите положительный и отрицательный тест
...
### 7. Каждый контекст должен отражать различие вложенной части от внешней
...
- `let` ленивый: значение вычисляется при первом обращении. Если подготовка — часть контекста и должна произойти до выполнения `it`, используйте `before` или `let!`, чтобы явно зафиксировать порядок и не нарушать этапы правила 8.

### 9. Используйте возможности FactoryBot для скрытия деталей подготовки
...
- `let` с фабрикой явно описывает характеристику (`report` со статусом и данными).`, атрибуты), а не SQL-обходные пути.
- При смене схемы адаптируем фабрику — тесты остаются декларативными и следуют глоссарию характеристик.

Это правило тесно связано с пунктами 1, 6 и 7: мы описываем поведение, проверяем обе стороны правила и держим подготовку рядом с контекстом.

## Язык описания спецификаций

### 11. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны составлять валидное предложение на английском языке.
...
### 12. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны быть написаны так, чтобы их понимал любой человек
...
### 13. Грамматика формулировок в describe/context/it
...
### 14. Язык контекстов: when / with / and / without / but / NOT
...
- Если в описании `it` появляется `when`/`with`/`and`/`without`/`but`, значит вы потеряли соответствующий контекст. Вынесите условие в `context`, иначе пример будет смешивать Given и Then и нарушит правила 1–7. Исключение — отрицательные формулировки с `does NOT`, где `NOT` подчёркивает результат, а не условие.
...
### 16. Не используйте [any_instance](https://rspec.info/features/3-13/rspec-mocks/old-syntax/any-instance/), allow_any_instance_of, expect_any_instance_of
...
### 17. Используйте `:aggregate_failures` только когда описываете одно правило
...
### 18. Предпочитайте verifying doubles (`instance_double`, `class_double`, `object_double`)
...
`shared_examples` служат для проверки общего поведения разных объектов. Они не про DRY ради снижения строк кода — тесты мы не «программируем» (см. пункт 10), они описывают правила. Если поведение повторяется, выносите именно его описание и ожидаемые наблюдения.
...
### 20. Предпочитайте Request (API-контракт) specs вместо controller specs
...
### 21. Стабилизируйте время через `ActiveSupport::Testing::TimeHelpers`
...
### 22. Делайте вывод падения теста читаемым
...
- Если matcher не даёт достаточной ясности, напишите helper, который вернёт компактное описание расхождения (но не превращайтесь в mini-программу — см. пункт 10).
