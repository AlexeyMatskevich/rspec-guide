...
- `:aggregate_failures` (правило 21) применяем, только если говорим об одном поведении и хотим увидеть все нарушения сразу — это не способ проверить половину пирамиды в одном `it`.
...
- Когда нужно проверить несколько следствий одного правила, разделяйте их на разные `it`. `:aggregate_failures` используйте лишь когда действительно говорите об одном поведении (см. пункт 21).
...
4. Вынесите инвариантные ожидания в `shared_examples` и подключите их через `it_behaves_like` (см. пункт 23.2).
...
### 15. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны составлять валидное предложение на английском языке.
...
### 16. Описание контекстов `context` и тестовых кейсов `it` вместе (включая `it`) должны быть написаны так, чтобы их понимал любой человек
...
### 17. Грамматика формулировок в describe/context/it
...
### 18. Язык контекстов: when / with / and / without / but / NOT
...
### 19. Изучите подробно правила из rubocop по части наименования https://rspec.rubystyle.guide/#naming

## Инструменты и поддержка тестов

### 20. Не используйте [any_instance](https://rspec.info/features/3-13/rspec-mocks/old-syntax/any-instance/), allow_any_instance_of, expect_any_instance_of
...
### 21. Используйте `:aggregate_failures` только когда описываете одно правило
...
### 22. Предпочитайте verifying doubles (`instance_double`, `class_double`, `object_double`)
...
### 23. Используйте shared examples для декларации контрактов
...
### 24. Предпочитайте Request (API-контракт) specs вместо controller specs
...
### 25. Стабилизируйте время через `ActiveSupport::Testing::TimeHelpers`
...
### 26. Делайте вывод падения теста читаемым
